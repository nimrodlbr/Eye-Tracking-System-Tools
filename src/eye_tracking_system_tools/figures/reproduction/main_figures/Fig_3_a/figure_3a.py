"""
Reproduction script for Figure 3a: Eye angle dynamics for 30 seconds during a behaviorally quiet epoch.
...
def main():
    """Main function to reproduce Figure 3a."""
    # Get the directory of this script
    script_dir = Path(__file__).parent
    
    # Load the pickle data
    pickle_file = script_dir / "figure_3a.pickle"
    output_path = script_dir / "figure_3a.pdf"
    
    try:
        with open(pickle_file, "rb") as f:
            data = pickle.load(f)
        print(f"Loaded data for {data.get('figure_name', 'figure_3a')}")
