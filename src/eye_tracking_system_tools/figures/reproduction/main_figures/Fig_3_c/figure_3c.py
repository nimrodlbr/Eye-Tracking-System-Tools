"""
Reproduction script for Figure 3c: Eye angles, pupil size, and rate of saccade and head movement events.
...
def main():
    """Main function to reproduce Figure 3c."""
    # Get the directory of this script
    script_dir = Path(__file__).parent
    
    # Load the pickle data
    pickle_file = script_dir / "figure_3c.pickle"
    output_path = script_dir / "figure_3c.pdf"
    
    try:
        with open(pickle_file, "rb") as f:
            data = pickle.load(f)
        print(f"Loaded data for {data.get('figure_name', 'figure_3c')}")
    ...
    traces = data.get('traces', ['center_x', 'center_y', 'pupil_diameter', 'saccade_frequency'])
    ...
    if behavior_state is not None:
        ...
        # Convert times to milliseconds if behavior_time_unit is 'ms' and times are in seconds
        if behavior_state_df is not None and figure_params.get('behavior_time_unit') == 'ms':
            # Check if times are in seconds (less than 10000) and convert to milliseconds
            if behavior_state_df['start_time'].max() < 10000:
                behavior_state_df['start_time'] = behavior_state_df['start_time'] * 1000
                behavior_state_df['end_time'] = behavior_state_df['end_time'] * 1000
    else:
        behavior_state_df = None
    ...
    print("Figure 3c reproduction complete!")
